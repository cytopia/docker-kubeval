...
_test-version:
	...
	@if [ "$(VERSION)" = "latest" ]; then \
		...
		if ! docker run --rm --platform $(ARCH) $(IMAGE):$(DOCKER_TAG) --version | grep -E "Version: $${LATEST}$$"; then \
		...
		if ! docker run --rm --platform $(ARCH) $(IMAGE):$(DOCKER_TAG) --version | grep -E "Version: $(VERSION)"; then \
			docker run --rm --platform $(ARCH) $(IMAGE):$(DOCKER_TAG) --version; \
			echo "Failed"; \
			exit 1; \
		fi; \
...
_test-run:
	@echo "------------------------------------------------------------"
	@echo "- Testing k8s files"
	@echo "------------------------------------------------------------"
	@if ! docker run --rm --platform $(ARCH) -v $(CURRENT_DIR):/data $(IMAGE):$(DOCKER_TAG) tests/*.yml ; then \
		echo "Failed"; \
		exit 1; \
	fi; \
	echo "Success";
